...
Once the logs are in Braintrust, go to [**<Icon icon="pentagon" /> Topics**](https://www.braintrust.dev/app/~/topics). Topics automatically classifies each conversation across three built-in facets:
...
![The Topics evaluation loop: get patterns from logs, save errors to a dataset, run offline evals, then implement the better hypothesis online](./assets/eval-loop.png)
