...
Generally, if you're using a model from OpenAI, you'd use function calling, defining functions with specific parameters, and letting the model decide when and how to call them.
If you want to use models from Anthropic, you'd need to take a different approach and use their Tool feature, or use prompt engineering to get the desired response.

In the Braintrust Playground, it's easy to use either AI provider with structured outputs by simply selecting **Structured output** from the output dropdown menu and defining a JSON schema.
...
Select **Scorers**, then **Create custom scorer**. We'll create a custom TypeScript scorer that compares the value of `output.is_spam` to the expected classification:
...
Navigate to the **Experiments** page to view your evaluation. Examine the scores generated by your evals. If you notice that some of your outputs did not match what was expected, you can tweak your prompt directly in the UI until it consistently produces high-quality outputs. If changing the prompt doesn't yield the desired results, consider experimenting with different models.
