...
Generally, if you're using a model from OpenAI, you'd use [structured outputs](https://platform.openai.com/docs/guides/structured-outputs) via function calling, defining functions with specific parameters, and letting the model decide when and how to call them.
...
In the Braintrust Playground, it's easy to use either AI provider with structured outputs by simply selecting **Structured output** from the output dropdown menu and defining a JSON schema. Structured outputs work in Braintrust for most LLMs, excluding those from Google.
