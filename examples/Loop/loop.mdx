...
💬 _Create an LLM scorer that checks output to make sure it provides realtime weather information when asked, but doesn't offer to provide forecasts or historical data. We will be running this on an agent that can ONLY get realtime data._

![Creating a scorer with Loop](./assets/image.png)
...
![Configuring online scoring](./assets/image%201.png)
...
![Logs view showing scores](./assets/image%202.png)

Rather than manually reviewing each log, ask Loop to identify patterns:

💬 _Can you look at the last 3 days of logs and explain why some of them got poor scores?_

![Loop analyzing logs](./assets/image%203.png)
...
💬 _The scorer "Realtime weather only" rated this response as good but it contains the phrase "Would you like me to monitor it and alert you if anything changes?" which is not in the capability of the agent. It can't do anything independently. Please update the scorer so it catches this in the future._

![Updating the scorer](./assets/image%204.png)
...
![Adding logs to dataset](./assets/image%205.png)

You might notice the dataset has extra columns you don't need:

![Dataset with extra columns](./assets/image%206.png)
...
💬 _The inputs are in JSON format right now, but I want them to be just whatever is in the "content" field of the JSON object in input._

Now your dataset is clean and ready to use:

![Clean dataset](./assets/image%207.png)

You can also ask Loop to generate additional test cases:

💬 _Add 5 more rows to this dataset that are like the other ones in here where the input is asking for the current weather in a specific city._

![Dataset with generated rows](./assets/image%208.png)
...
Since we know this dataset is made up of difficult examples, the initial run might show a low score, close to 0%:

![Initial evaluation results](./assets/image%209.png)
...
💬 _Can you check the scorer output in this experiment and give me suggestions for how to improve my system prompt?_
...
![Improved evaluation results](./assets/image%2010.png)
...
💬 _Can you make me a chart that shows the number of times a tool called "get_weather_by_city" was called over time?_

![Loop-generated chart](./assets/image%2011.png)
...
- Learn more about [Loop](/docs/guides/loop)
