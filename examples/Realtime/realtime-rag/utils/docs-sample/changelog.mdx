...
- You can now export your prompts from the playground as code snippets and run them through the [AI proxy](/docs/deploy/ai-proxy).
...
- Support for creating and pushing custom Python tools and prompts from your codebase with `braintrust push`. Read the guides to [tools](/docs/deploy/functions) and [prompts](/docs/evaluate/write-prompts) for more information.
...
- The Braintrust [AI Proxy](/docs/deploy/ai-proxy) now supports the [OpenAI Realtime API](https://platform.openai.com/docs/guides/realtime), providing observability for voice-to-voice model sessions and simplifying backend infrastructure.
- Add "Group by" functionality to the monitor page.
- The experiment table can now be visualized in a [grid layout](/docs/evaluate/interpret-results#grid-layout), where each column represents an experiment to compare long-form outputs side-by-side.
...
- The Braintrust [AI Proxy](/docs/deploy/ai-proxy) can now [issue temporary credentials](/docs/deploy/ai-proxy#api-key-management) to access the proxy for a limited time. This can be used to make AI requests directly from frontends and mobile apps, minimizing latency without exposing your API keys.
...
- Preview [file attachments](/docs/instrument/custom-tracing#uploading-attachments) in the trace view.
...
- You can now export [OpenTelemetry (OTel)](https://opentelemetry.io/docs/specs/otel/) traces to Braintrust. See
  the [tracing guide](/docs/instrument/custom-tracing#opentelemetry-otel) for more details.
...
- The [Github action](/docs/evaluate/run-evaluations#github-action) now supports Python runtimes.
- Add support for [Cerebras](https://cerebras.ai/) models in the proxy, playground, and saved prompts.
- You can now create [span iframe viewers](/docs/instrument/custom-tracing#custom-span-iframes) to visualize span data in a custom iframe.
...
- Create custom tools to use in your prompts and in the playground. See the [docs](/docs/evaluate/write-prompts#calling-external-tools) for more details.
...
- Create and push custom tools from your codebase with `braintrust push`. See [docs](/docs/evaluate/write-prompts#calling-external-tools) for more details. TypeScript only for now.
...
- You can now create server-side online evaluations for your logs. Online evals support both [autoevals](/docs/reference/autoevals) and
  [custom scorers](/docs/evaluate/playgrounds) you define as LLM-as-a-judge, TypeScript, or Python functions. See
  [docs](/docs/evaluate/run-evaluations#online-evaluation) for more details.
...
- Update the `full` docker deployment configuration to bundle the metadata DB
  ...
  [docs](/docs/admin/self-hosting/docker#full-configuration) for details. If
...
- [`updateSpan()`/`update_span()`](/docs/instrument/custom-tracing#updating-spans) allows you to update a span's fields after it has been created.
...
- Add support for labeling [expected fields using human review](/docs/annotate/human-review#writing-categorical-scores-to-expected-field).
...
- [Custom comparison keys](/docs/evaluate/interpret-results#customizing-the-comparison-key) for experiments.
...
- Added functionality for distributed tracing. See the
  [docs](/docs/instrument/custom-tracing#distributed-tracing) for more details.
...
- You can now return dynamic arrays of scores in `Eval()` functions ([docs](/docs/evaluate/run-evaluations#dynamic-scoring)).
- Launched [Reporters](/docs/evaluate/run-evaluations#custom-reporters), a way to summarize and report eval results in a custom format.
...
- Shipped support for [prompt management](/docs/evaluate/write-prompts).
...
- Images now supported in the tree view (see [tracing docs](/docs/instrument/custom-tracing#multimodal-content) for more).
...
- Add support for [tags](/docs/instrument/custom-tracing#tags-and-queues).
...
- Support for [custom models](/docs/evaluate/playgrounds#custom-models) in the playground.
...
- Added an example of [closed QA / extra fields](/docs/evaluate/run-evaluations#additional-fields).
...
- Published new [guide for tracing and logging your code](/docs/instrument/custom-tracing)
...
- Released new [trials](/docs/evaluate/run-evaluations#trials) feature to rerun each input multiple times and collect aggregate results for a more robust score.
...
- Modified Autoevals library to use the [AI proxy](/docs/deploy/ai-proxy)
...
- Released [AI proxy](/docs/deploy/ai-proxy): access many LLMs using one API w/ caching
- Added [load balancing endpoints](/docs/deploy/ai-proxy#load-balancing) to AI proxy
...
- [Launched new tracing feature: log and visualize complex LLM chains and executions.](/docs/evaluate/run-evaluations#tracing)
...
- The Eval framework is now supported in Python! See the updated [evals guide](/docs/evaluate/run-evaluations) for more information:
...
- The prompt playground is now live! We're excited to get your feedback as we continue to build
  this feature out. See [the docs](/docs/evaluate/playgrounds) for more information.
...
- The [eval CLI](/docs/evaluate/run-evaluations) now supports `--watch`, which will automatically re-run your evaluation when you make
...
- Introducing datasets! You can now upload datasets to Braintrust and use them in your experiments. Datasets are
  ...
  performance against a baseline. Learn more about [how to create and use datasets in the docs](/docs/annotate/datasets).
...
- A new [framework](/docs/evaluate/run-evaluations) for expressing evaluations in a much simpler way:
