...
For JavaScript/TypeScript applications, you can use the `BraintrustSpanProcessor` which handles all the configuration automatically:
...
const sdk = new NodeSDK({
  spanProcessors: [
    new BraintrustSpanProcessor({
      apiKey: "<Your API Key>",
      parent: "project_id:<Your Project ID>",
...
Alternatively, you can set environment variables and the processor will use them automatically:
...
For other collectors that use the [OpenTelemetry SDK](https://opentelemetry.io/docs/languages/) to export traces manually, set the
