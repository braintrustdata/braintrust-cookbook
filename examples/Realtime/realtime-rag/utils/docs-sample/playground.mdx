...
[datasets](/docs/annotate/datasets).
...
![Empty Playground](/docs/evaluate/playgrounds/empty-playground.webp)

### Sharing playgrounds

Playgrounds are designed for collaboration and automatically synchronize in real-time.

![Sync Playground](/docs/evaluate/playgrounds/sync-playground.gif)
...
![Dataset dropdown](/docs/evaluate/playgrounds/prompt-dataset-dropdown.webp)
...
![Prompt with dataset](/docs/evaluate/playgrounds/prompt-with-dataset.webp)

Each value can be arbitrarily complex JSON, e.g.

![Prompt with JSON data](/docs/evaluate/playgrounds/prompt-with-dataset-json.webp)
...
![Multimodal prompt](/docs/evaluate/playgrounds/multimodal-prompt.png)

### Prompt code snippets

The playground makes it easy to copy code snippets that you can run through the [AI proxy](/docs/deploy/ai-proxy). Select the code icon (<Code className="inline size-3" />) next to any chat-based prompt to get code snippets in TypeScript, Python, or cURL.
...
To configure custom models, see the [Custom models](/docs/deploy/ai-proxy#custom-models) section of the proxy docs.
