...
Several features in Braintrust are powered by the proxy. For example, when you create a [playground](/docs/evaluate/playgrounds),
the proxy handles running the LLM calls. Similarly, if you [create a prompt](/docs/evaluate/write-prompts), when you preview the
...
within your own environment. See the [self-hosting](/docs/admin/self-hosting) docs for more information on how to set up
self-hosting.
