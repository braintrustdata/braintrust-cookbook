...
While developing a prompt, it can be useful to test it out on real-world data in the [Playground](/docs/evaluate/playgrounds).
...
derive from `span.export()` while doing [distributed tracing](/docs/instrument/custom-tracing#distributed-tracing).
...
<Callout type="info">
  ...
  proxy](/docs/deploy/ai-proxy) to access a wide range of models using the OpenAI
