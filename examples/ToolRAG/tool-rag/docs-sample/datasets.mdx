...
- **Secure**. If you run Braintrust [in your cloud environment](/docs/admin/self-hosting), datasets are stored in your warehouse and
...
![Dataset Viewer](/docs/annotate/datasets/datasets.webp)
...
configure [categorical fields](/docs/annotate/human-review#writing-categorical-scores-to-expected-field) to allow human reviewers
...
For a more advanced overview of how to use an experiment as a baseline for other experiments, see [Hill climbing](/docs/evaluate/run-evaluations#hill-climbing).
...
can [paginate](/docs/deploy/api#downloading-a-dataset-using-pagination) through the dataset to download it in smaller chunks.
