...
Each cell is a configuration's success on one suite. Scan a row: all green is a genuine generalist, while a single red cell in an otherwise green row is the suite where that configuration will fail you in production.
