...
The source uses the OpenTelemetry GenAI "parts" format, which the script normalizes to OpenAI-style messages so Braintrust renders them as readable chat turns. For example, an assistant turn that calls a tool arrives as a list of typed parts:

```json
{
  "role": "assistant",
  "parts": [
    { "type": "text", "content": "Let me look up that order." },
    { "type": "tool_call", "id": "call_1", "name": "get_order", "arguments": { "order_id": "A123" } }
  ]
}
```

and the script rewrites it into the OpenAI shape Braintrust expects (text in `content`, calls in `tool_calls`):
  ...
  "tool_calls": [
    {
      "id": "call_1",
      "type": "function",
      "function": { "name": "get_order", "arguments": "{\"order_id\": \"A123\"}" }
    }
  ]
}
```

Run the script once to preview, then again with `--push` to upload:
...
Analyzing traces tells you where the agent fails. To check whether a fix actually helps, you need a gradable [dataset](/docs/annotate/datasets/index) and an experiment. The cookbook's second script, [`hf_bt_cookbook/import_dataset.py`](https://github.com/braintrustdata/braintrust-cookbook/blob/main/examples/HuggingFaceAgentTraces/hf_bt_cookbook/import_dataset.py), turns Hugging Face rows into a Braintrust **Dataset** of `{input, expected, metadata}` records. You rewrite one function, `to_record`, which is the entire mapping:
...
- Run [experiments](/docs/evaluate/run-evaluations) against the configurations your analysis flagged.
