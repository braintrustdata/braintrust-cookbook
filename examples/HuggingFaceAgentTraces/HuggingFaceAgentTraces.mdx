...
Nothing leaves your machine until you pass `--push`. And it's important to note here that root span IDs are deterministic (`uuid5` of the session ID), so re-importing the same session upserts instead of duplicating.
