# Using Topics to find AI failures that testing misses
...
Topics is a Braintrust feature that automatically analyzes your logged traces and classifies them into meaningful clusters. Here's how it works:
1. Each logged conversation (thread) is first passed through a preprocessor that extracts the relevant content
2. Then an LLM generates a short summary of what happened in that conversation. 
3. Those summaries are then classified into clusters across multiple dimensions called facets. 

<image here of the preprocessor thread>

Out of the box, Topics ships with three built-in facets: **Task** (what the user was trying to accomplish), **Sentiment** (how the user felt during the interaction), and **Issues** (whether something went wrong). In this cookbook, we'll use those facets to run offline evals, fix the prompt, and improve our overall system.

By the end, you'll learn how to:

- Set up a tool-calling customer support chatbot with real API backends
- Generate realistic conversation logs and send them to Braintrust
...
### Setup and code explaination

We have a Supabase database has six tables:
...
Each conversation is logged to Braintrust with full tracing. That means every tool call, LLM response, each conversational turn is logged to Braintrust.

<insert image of a full trace>
...
<image of topics homepage (before generating)>

### What the classifications revealed

With ~200 traces logged, Topics surfaced several clusters. 

On the Task dimension, conversations grouped naturally around the scenario categories we expected like shipping, returns, billing, account, and product inquiries. but Topics also identified cross-cutting clusters like "Product sizing and returns" that combined sizing questions with return requests, which we hadn't explicitly designed for. 

Sentiment had a bunch of "Negative emotional distress" even though the bot's Brand Alignment scores were high, meaning the bot sounded on-brand but was still leaving customers frustrated. 

On the Issues facet, clusters around "Payment processing failures" and "Order lookup errors" started forming once we had enough traces where tool calls failed.

<Image of the final cluster results>
...
At this point, I would recommend spending a good 20-30 minutes manually reading through the logs. Though you could technically use AI to automate this portion of it, it's better to have a human read through and decide which AI responses were fine (even if it evoked negative emotion) versus which ones fundamenetally need to be fixed. As I did this myself, I wrote down what needed to be fixed in my system.

**1. Order not found = dead end.** When a customer referenced an order number that wasn't in the system, the bot just said "order not found" and stopped. I think the correct behavior should be to look up the customer's account to find valid orders numbers, or find other order numbers that are close to what the customer provided, in case they made a typo.
...
<image of me writing this down issues manually>
